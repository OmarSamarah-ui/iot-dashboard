...
const Settings = ({ theme, setTheme }: { theme: string; setTheme: (theme: string) => void }) => {
    const toggleTheme = () => {
    ...
    return (
        <motion.div className='p-6' initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.7 }}>
            ...
            <div className='flex items-center rounded-lg'>
                ...
                <button title='Change theme' onClick={toggleTheme} className={`relative w-14 h-7 rounded-full p-1 transition-all duration-300 ${theme === 'dark' ? 'bg-blue-500' : 'bg-gray-300'}`}>
